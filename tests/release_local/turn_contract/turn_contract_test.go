...
// ── Fork divergence: context-indicator format ──────────────────
//
// Upstream renders the context indicator as the bare "[ctx: ~N%]".
// This fork's token usage indicator (docs/fork-features.md, 9e7edf09)
// deliberately replaces it with "[<dur> · <in> in · <out> out · ctx ~N%]",
// which carries turn duration and token counts alongside the percentage.
//
// The contract these tests guard — exactly one indicator, rendered once, at
// the tail of the final reply — is unchanged, so the assertions match on the
// indicator's stable marker rather than upstream's exact prefix.

// ctxMarker is the substring common to both the upstream and fork renderings.
const ctxMarker = "ctx"

// countCtxIndicators counts rendered context indicators in s. Both formats put
// the indicator in a trailing bracketed group containing "ctx ~N%" / "ctx: ~N%".
func countCtxIndicators(s string) int {
	n := 0
	for _, seg := range strings.Split(s, "[") {
		if i := strings.Index(seg, "]"); i >= 0 {
			if inner := seg[:i]; strings.Contains(inner, ctxMarker+":") || strings.Contains(inner, ctxMarker+" ~") {
				n++
			}
		}
	}
...
// hasCtxIndicator reports whether s carries a context indicator in either format.
func hasCtxIndicator(s string) bool { return countCtxIndicators(s) > 0 }
...
func TestBasicUserTurnContractAcrossInputModalities(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if countCtxIndicators(texts[0]) != 1 {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantPreview {
				...
				if !hasCtxIndicator(updates[len(updates)-1]) {
			...
			if len(texts) != 1 || !strings.Contains(texts[0], strings.TrimSpace(body)) || countCtxIndicators(texts[0]) != 1 {
...
func TestLongFinalResponseKeepsMetadataOnceAtTail(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		if len(texts) >= 2 {
			joined := strings.Join(texts, "")
			if countCtxIndicators(joined) != 1 || strings.Count(joined, "glm-5.1") != 1 {
				t.Fatalf("chunks = %#v, want metadata exactly once", texts)
			}
			if !(hasCtxIndicator(texts[len(texts)-1]) && strings.Contains(texts[len(texts)-1], "glm-5.1")) {
